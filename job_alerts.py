...
WORKDAY = [
    ...
    # --- confirmed from Capital One's own careers page (migrated to wd12)
    ("Capital One",     "capitalone", "wd12", "Capital_One"),
    ("Salesforce",      "salesforce", "wd12", "External_Career_Site"),
    # Everything else guessed here failed --verify on 2026-08-11 and was
    # removed. Use find_boards.py to discover the real slug for any company
    # you want back, then add it as (Name, tenant, host, site).
    # Removed: AdventHealth, Eaton, RELX, UnitedHealth, CVS, Humana,
    # Elevance, Target, 3M, Cargill, Mastercard, Amex, Schwab, Fidelity,
    # Dell, NVIDIA, Deloitte, KPMG, Nationwide, Progressive, American
    # Airlines, Optum.
]
...
def fetch_google(name="Google"):
    """Google's careers site has a public JSON search endpoint."""
    seen, out = set(), []
    for term in SEARCH_TERMS:
        try:
            r = requests.get(
    ...
    for term in SEARCH_TERMS[:3]:
        try:
            r = requests.post(
                ...
                json={"query": term, "filters": {"postingpostLocation": ["postLocation-USA"]},
...
ORACLE_CLOUD = [
    ("GuideWell/Florida Blue", "fa-etum-saasfaprod1", "CX_1"),
]


def fetch_oracle_cloud(name, host, site):
    """Oracle Recruiting Cloud public candidate-experience API."""
    url = (f"https://{host}.fa.ocs.oraclecloud.com/hcmRestApi/resources/latest/"
        ...
        try:
            ...
            for j in items:
                ...
                out.append({
                    ...
                    "url": (f"https://{host}.fa.ocs.oraclecloud.com/hcmUI/"
                            f"CandidateExperience/en/sites/{site}/job/{jid}"),
                    "description": " ".join(filter(None, [
...
def fetch_apify(name="LinkedIn/Indeed (Apify)"):
    ...
    if not APIFY_TOKEN:
        raise RuntimeError("APIFY_TOKEN not set - skipping")
    now = datetime.now()
...
SOURCES = ([("oracle_cloud", fetch_oracle_cloud, a) for a in ORACLE_CLOUD]
           ...
           + [("amazon", fetch_amazon, ()),
              ("adzuna", fetch_adzuna, ()), ("apify", fetch_apify, ())]
