...
# STRICT: a posting must carry POSITIVE evidence of being in the US.
# The old blacklist approach let anything through whose country wasn't on the
# list (and let blanks through), which is how Singapore and Malaysia slipped in.
STRICT_US = True
...
US_MARKERS = (
    ["united states", "usa", "u.s.a", "u.s.", " us ", "america",
     "remote - us", "remote, us", "remote usa", "us remote", "nationwide"]
    + list(US_STATES)                                   # full state names
)
US_ABBREV = set(US_STATES.values())

# A handful of US cities that often appear with no state attached.
US_CITIES = [
    ...
    "kansas city", "omaha", "raleigh", "charlotte", "pittsburgh",
    "cincinnati", "cleveland", "columbus", "indianapolis", "nashville",
    "memphis", "st. louis", "saint louis", "salt lake city", "tampa",
    "orlando", "oklahoma city", "tulsa", "san jose", "mclean",
    "richmond", "plano", "irving", "sunnyvale", "cupertino", "redmond",
    "mountain view", "menlo park", "santa clara", "bentonville",
    "rochester", "hopkins", "saint paul", "st. paul", "sylmar",
]


def is_us(location):
    """True only when the location shows positive evidence of being in the US."""
    if not US_ONLY:
        return True
    loc = (location or "").strip()
    if not loc:
        return not STRICT_US        # blank: drop in strict mode
    low = loc.lower()

    # "Remote" on its own is ambiguous; allow it, it's usually US on US boards.
    if low in ("remote", "remote - usa", "us", "united states"):
        return True

    for m in US_MARKERS:
        if re.search(rf"\b{re.escape(m.strip())}\b", low):
            return True
    for c in US_CITIES:
        if re.search(rf"\b{re.escape(c)}\b", low):
            return True
    # Two-letter state code, e.g. "Irving, TX" or "Minneapolis MN 55401"
    for tok in re.findall(r"\b[A-Z]{2}\b", loc):
        if tok in US_ABBREV:
            return True
    # "N Locations" style multi-site postings - can't tell, keep them.
    if re.match(r"^\d+\s+locations?$", low):
        return True
    return False
